 ...
 */
	...
	//$ccd_list = isset($_SESSION['coming_show']['ccd_list'])?$_SESSION['coming_show']['ccd_list']:array();
	// Parameter List
	$param_list = isset($_SESSION['coming_show']['param_list'])?$_SESSION['coming_show']['param_list']:array();
	...
	$_SESSION['coming_show']['ccd_list'] = $ccd_list;
	$_SESSION['coming_show']['param_list'] = $param_list;
	...
	if (isset($action)) {
		switch($action) {
			case 'resetLocation':
				$smarty->display(ROOT.'/templates/show/coming_search.tpl');
				break;
			case 'resetCriteria':
				$smarty->display(ROOT.'/templates/show/coming_result.tpl');
				break;
			default:
				break;
		}
	} else {
		$smarty->display(ROOT.'/templates/show/coming.tpl');
	}
...
//------------------------------------- functions ------------------------------------
	...
	function initParams() {
		...
		if (isset($_REQUEST['cs']) && $_REQUEST['cs'] = 'cs') {
			...
			if (!isset($_SESSION['coming_show']['param_list'])) {
				$param_list['city_id'] = $_SESSION['city_id'];
				$param_list['district_id'] = -1;
				$param_list['organizer_id'] = -1;
				$param_list['venue_id'] = -1;
